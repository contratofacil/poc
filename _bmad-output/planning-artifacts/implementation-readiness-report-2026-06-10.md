...
4. ~~**Cookie Consent Banner (CMP)**~~ — ✅ **DÉJÀ IMPLÉMENTÉ** (vérifié 2026-06-11) : `apps/frontend/src/lib/consent/` + `ConsentBanner.tsx` intégré au layout racine. Conforme à la spec : 4 catégories, Accepter/Refuser à équivalence visuelle, modal accessible (focus trap, ARIA), persistance 12 mois, versioning, lien footer
5. ~~**KYC / eIDV dans le wizard NIF**~~ — ✅ **IMPLÉMENTÉ EN SIMULATION (POC)** le 2026-06-11 : étape 3 « Vérification d'identité » dans le wizard NIF (FR/PT, vocabulaire neutre, disclaimer Lei 83/2017), backend `routes/kyc.ts` avec driver `KYC_DRIVER=mock` (eIDV simulée, screening PEP simulé → blocage + escalade cabinet + statut neutre anti tipping-off), table `kyc_verifications` avec rétention 7 ans, 8 tests Jest. **Décision OQ-007 actée : Veriff retenu pour la production** (« Privy KYC » n'existe pas — privy.io n'offre pas de produit eIDV) ; l'intégration Veriff réelle reste à câbler avant lancement (driver `veriff` → 501 en attendant)
6. ~~**Disclaimers + checkbox rétractation 14 j au checkout contrat**~~ — ✅ **IMPLÉMENTÉ** le 2026-06-11 : bandeau permanent de responsabilité + checkbox de vérification + checkbox de renonciation au droit de rétractation (art. 16(m) Dir. 2011/83/UE) dans le wizard contrat, FR/PT, blocage de la finalisation tant que non cochées — vérifié en navigateur
...
| US-1.5 — Cookie Consent CMP | 🔴 Critique | ✅ **Déjà implémenté** — `lib/consent/` + `ConsentBanner` dans le layout, conforme spec (vérifié 2026-06-11) |
| US-1.6 — In-App Notification Center | 🟠 Important | ❌ À faire — post-lancement acceptable |
| US-2.5 — KYC / eIDV | 🔴 Critique | 🟠 **Simulé (POC)** — flow complet + PEP + rétention 7 ans en mock ; OQ-007 résolu (Veriff) ; intégration réelle à câbler avant prod |
...
**Bilan : 4 des 10 stories « critiques » sont déjà codées (auth Privy, CMP, prestation de services, pipeline RAG).** Restent 2 bloquants légaux (KYC/eIDV, disclaimers checkout), 1 vérification (procuration NIF), et 3 éléments différables post-lancement.
...
Les vrais bloquants de lancement sont au nombre de deux : **KYC/eIDV** (Lei 83/2017) et **disclaimers checkout** (Dir. 2011/83/UE) — la CMP cookies étant déjà livrée et conforme. S'y ajoutent le passage de Stripe et SendGrid en mode réel et l'infrastructure de production. Estimation : **~1,5 à 2 semaines jusqu'au lancement.**
